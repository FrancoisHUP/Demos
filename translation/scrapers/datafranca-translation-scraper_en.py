...
with open('words_with_links_en.json', 'r', encoding='utf-8') as f:
    words_data = json.load(f)

translated_words = []

# Load the last processed index
if os.path.exists('last_processed_index_en.txt'):
    with open('last_processed_index_en.txt', 'r') as f:
...
def save_progress(translated_words, index):
    with open('translated_words_en.json', 'w', encoding='utf-8') as f:
        json.dump(translated_words, f, ensure_ascii=False, indent=4)
    with open('last_processed_index_en.txt', 'w') as f:
        f.write(str(index))
...
print("Translation completed and data saved to translated_words_en.json")
